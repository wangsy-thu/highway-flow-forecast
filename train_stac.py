...
from models.stac import make_model
...
def train_main():
    ...
    optimizer = optim.Adam(stac_net.parameters(), lr=learning_rate)  # Adam优化器
    ...
    if start_epoch > 0:
        ...
        stac_net.load_state_dict(torch.load(params_file_name, map_location=DEVICE))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            torch.save(stac_net.state_dict(), params_file_name)
            print('Save Model to: {}'.format(params_path))

        stac_net.train()  # 恢复训练模式，开启梯度更新
        ...
        for batch_data in tqdm(train_loader, desc='Epoch: {}'.format(epoch + 1)):
            ...
            outputs = stac_net(encoder_input)  # 正向传播
            # 计算损失
            if masked_flag:
...
def predict_main(global_step: int, data_loader: DataLoader,
    ...
    stac_net.load_state_dict(torch.load(params_file_name, map_location='cpu'))
