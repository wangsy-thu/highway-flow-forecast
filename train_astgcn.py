...
from utils.chebyshev_conv import get_adjacency_matrix
...
chebyshev_channels = int(training_config['nb_chev_filter'])
time_channels = int(training_config['nb_time_filter'])
...
adj_mat, _ = get_adjacency_matrix(
...
def train_main():
    ...
    optimizer = optim.Adam(astgcn_net.parameters(), lr=learning_rate)  # Adam优化器
    ...
    if start_epoch > 0:
        ...
        astgcn_net.load_state_dict(torch.load(params_file_name, map_location=DEVICE))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            torch.save(astgcn_net.state_dict(), params_file_name)
            print('Save Model to: {}'.format(params_path))

        astgcn_net.train()  # 恢复训练模式，开启梯度更新
        ...
        for batch_data in tqdm(train_loader, desc='Epoch: {}'.format(epoch + 1)):
            ...
            outputs = astgcn_net(encoder_input)  # 正向传播
            # 计算损失
            if masked_flag:
...
def predict_main(global_step: int, data_loader: DataLoader,
    ...
    astgcn_net.load_state_dict(torch.load(params_file_name, map_location='cpu'))
