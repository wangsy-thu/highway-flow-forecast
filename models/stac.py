...
class STACGCNBlock(nn.Module):
    """
    STACGCN Block
    这里定义一个 STACGCN 块，后续可以通过块堆叠增加模型表达能力
    """

    def __init__(self, in_channels: int, time_channels: int, time_strides: int,
                 vertices_num: int, time_step_num: int):
        """
        构造函数
        ...
        """
        super(STACGCNBlock, self).__init__()
        # 时间注意力层
    ...
    def forward(self, x_matrix):
        ...
        # 时间卷积
        x_mat_time_conv = self.TimeConv(x_mat_s_att_norm.permute(0, 2, 1, 3))  # (B, N, time_channels, T)
...
class STACGCN(nn.Module):
    """
    STACGIN whole module
    STACGIN 完整模型
    """

    def __init__(self, block_num: int, in_channels: int,
...
def make_model(
    ...
    print('Unused params:[K: {}, spatial_channels: {}, edge_index: {}]'.format(
        K, spatial_channels, edge_index.size()
    ))
    model = STACGCN(
        block_num=block_num,
        in_channels=in_channels,
