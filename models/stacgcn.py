...
class GCNConvLayer(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        """
        构造函数
        :param K: GCN网络层数，消息聚集邻居的跳数，默认为 2
        ...
        # 多层 GAT 卷积
        self.gcnConvList = nn.ModuleList([
            GCNConv(
                in_channels=in_channels,
                out_channels=out_channels
            )
        ])
        self.gcnConvList.extend([
            GCNConv(
    ...
    def forward(self, graph_sig):
        ...
        """
        for gcnLayer in self.gcnConvList:
            graph_sig = gcnLayer(graph_sig, self.edge_index)
        return graph_sig
...
class STACGCN(nn.Module):
    ...
    def __init__(self, block_num: int, in_channels: int, K: int,
        ...
        """
        构造函数
        :param block_num: STACGCN 块数量
        :param in_channels: 入通道数，这里是 Policy Conv后的通道数
        :param K: GCN 网络层数，消息聚集邻居的跳数，默认为 2
